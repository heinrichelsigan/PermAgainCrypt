    ...
    public class CFile : CMsg, IMsgAble
    {
        ...
        public override string EncryptToJson(string serverKey, EncodingType encoder = EncodingType.Base64, Zip.ZipType zipType = Zip.ZipType.None)
        {
            ...
            if (string.IsNullOrEmpty(serializedJson))
                throw new CException($"override string EncryptToJson(string serverKey) failed");
            ...
            EncodingType decoder = EncodingType.Base64, Zip.ZipType zipType = Zip.ZipType.None)
        {
            ...
            if (cfile == null)
                throw new CException($"override File? DecryptFromJson(string serverKey, string serialized) failed");                
            ...
            EncodingType encoder = EncodingType.Base64, Zip.ZipType zipType = Zip.ZipType.None)
        {
            if (string.IsNullOrEmpty(serverKey) || cfile == null)
                throw new CException($"static string ToJsonEncrypt(string serverKey, ref CFile cfile) failed: NULL reference!");
        ...
        )
        {
            if (string.IsNullOrEmpty(serialized))
                throw new CException("static CFile FromJsonDecrypt(string serverKey, string serialized): serialized is null or empty.");
            ...
            {
                throw new CException($"static CFile FromJsonDecrypt(string serverKey, string serialized) failed.");
            }
        ...
        )
        {
            ...
            try
            {
                ...
                {
                    throw new CException($"CFile.Hash={cfile.Hash} doesn't match PipeString={pipeString}");
                }                    
                ...
                if (!sha256Hash.Equals(cfile.Sha256Hash))
                {
                    ...
                    // throw new CException(sha256ErrMsg);
                }
