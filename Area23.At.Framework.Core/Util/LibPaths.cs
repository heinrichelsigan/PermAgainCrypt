    ...
    public static class LibPaths
    {
        ...
        static LibPaths()
        {
            ...
            if (Constants.DirCreate)
            {
                ...
                if (Directory.Exists(SytemDirUuPath))
                    try
                    {
                        Directory.CreateDirectory(SytemDirUuPath);
                    }
                    catch { }
                if (Directory.Exists(SystemDirOutPath))
                    try
                    {
                        Directory.CreateDirectory(SystemDirOutPath);
                    }
                    catch { }
                if (Directory.Exists(SystemDirTmpPath))
                    try
                    {
                        Directory.CreateDirectory(SystemDirTmpPath);
                    }
                    catch { }
                if (Directory.Exists(SystemDirLogPath))
                    try
                    {
                        Directory.CreateDirectory(SystemDirLogPath);
                    }
                    catch { }
            }
        }
            ...
            get
            {
                if (string.IsNullOrEmpty(baseAppPath))
                {
                    ...
                    if (SepCh == '/' &&
                        ...
                        basApPath = ConfigurationManager.AppSettings[Constants.BASE_APP_PATH_UNIX];
                    else if (ConfigurationManager.AppSettings[Constants.BASE_APP_PATH_WIN] != null)
                        basApPath = ConfigurationManager.AppSettings[Constants.BASE_APP_PATH_WIN];
            ...
            get
            {
                if (string.IsNullOrEmpty(cqrSrvSoap))
                {
                    if (ConfigurationManager.AppSettings[Constants.CQR_SRV_SOAP] != null)
                        cqrSrvSoap = ConfigurationManager.AppSettings[Constants.CQR_SRV_SOAP].ToString();
                    else
            ...
            get
            {
                if (string.IsNullOrEmpty(cqrSrvSoap12))
                {
                    if (ConfigurationManager.AppSettings[Constants.CQR_SRV_SOAP12] != null)
                        cqrSrvSoap12 = ConfigurationManager.AppSettings[Constants.CQR_SRV_SOAP12].ToString();
                    else
