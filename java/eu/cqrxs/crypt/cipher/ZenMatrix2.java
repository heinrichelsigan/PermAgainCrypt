...
public class ZenMatrix2 extends ZenMatrix implements BlockCipher  {
    ...
    public void reset() {
		byte sbcnt = 0x0;
        matrixPermutationKey = new byte[0x100];
        for (byte s : matrixPermutationBase2)  {
            privateBytes[sbcnt % 0x100] = (byte)0x0;
            matrixPermutationKey[sbcnt++] = s;
        }
        _inverseMatrix2 = buildInverseMatrix(matrixPermutationKey2, 0x100);
    ...
    public int processBlock2(byte[] inBuf, int inOff, byte[] outBuf, int outOff)
        ...
        if (inOff < inBuf.length && inOff + BLOCK_SIZE <= inBuf.length && outOff < outBuf.length && outOff + BLOCK_SIZE <= outBuf.length) {
            ...
            for (aCnt = 0, bCnt = 0; aCnt < BLOCK_SIZE; aCnt++)  {
                ...
                byte sm = forEncryption ? matrixPermutationKey2[aCnt % 0x100] : _inverseMatrix2[aCnt % 0x100];
    ...
    public ZenMatrix2(String secretKey, String hashIV, boolean fullSymmetric) {
        ...
        byte[] keyBytes = CryptHelper.getUserKeyBytes(secretKey, hashIV, 0x100);
