...
namespace Area23.At.WinForm.CryptFormCore.Gui.Forms
{
    ...
    public partial class EncryptFormMultiControls : EncryptFormBase
    {
        ...
        protected internal async Task Decrypt_Click(object sender, EventArgs e)
        {
            ...
            BitmapPipelineGnerator bGen = new BitmapPipelineGnerator(cPipe);
            await groupBoxFiles.pictureBoxRunningPipe.SetImageTagVisibleAsync(bGen.GenerateDecryptPipeImage());
            ...
            if (!string.IsNullOrEmpty(groupBoxFiles.labelFileIn.Text) && !groupBoxFiles.labelFileIn.Text.StartsWith("["))
            {
                ...
                try
                {
                    ...
                    if (saved)
                    {
                        ...
                        // await groupBoxFiles.SetLabelVisibleTextAsync(groupBoxFiles.labelOutputFile, true,);
