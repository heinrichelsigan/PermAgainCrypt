...
namespace Area23.At.WinForm.CryptFormCore.Gui.Forms
{
    ...
    public partial class EncryptFormSimple : EncryptFormBase
    {
        ...
        /// <summary>
        /// EncryptFormSimple_Load - form load event
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        internal void EncryptFormSimple_Load(object sender, EventArgs e)
        {
        ...
        protected internal async Task Encrypt_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(this.labelFileIn.Text) && !labelFileIn.Text.StartsWith("["))
            {
                ...
                try
                {
                    ...
                    bool saved = (menuFileSettingsItemAutomaticallySaveToTemp != null && menuFileSettingsItemAutomaticallySaveToTemp.Checked) ?
                                    SaveBytesWithoutDialog(fileBytes, ref outFilePath) : SaveBytesDialog(encodedBytes, ref outFilePath);
                    if (saved)
                    {
        ...
        protected internal async Task Decrypt_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(this.labelFileIn.Text) && !labelFileIn.Text.StartsWith("["))
            {
                ...
                try
                {
                    ...
                    bool saved = (menuFileSettingsItemAutomaticallySaveToTemp != null && menuFileSettingsItemAutomaticallySaveToTemp.Checked) ?
                                    SaveBytesWithoutDialog(fileBytes, ref outFileDecrypt) : SaveBytesDialog(fileBytes, ref outFileDecrypt);
                    if (saved)
                    {
        ...
        /// <summary>
        /// SaveBytesWithoutDialog saves byte array to a file in temporary directory
        /// </summary>
        /// <param name="fileBytes">byte array to save</param>
        /// <param name="outFilePath">ref will be returned; calculated outFilePath</param>
        /// <returns>true if saved, false if not saved</returns>
        internal bool SaveBytesWithoutDialog(byte[] fileBytes, ref string outFilePath)
        {
            // this.pictureBoxRunningPipe.Visible = false;
            SaveFileDialog dialog = new SaveFileDialog();
            outFilePath = outFilePath ?? string.Empty;
            if (fileBytes != null && fileBytes.Length > 0)
            {
                string appDataTemp = Environment.GetEnvironmentVariable("LOCALAPPDATA").ToString();
                if (!Directory.Exists(appDataTemp))
                {
                    appDataTemp = System.Windows.Forms.Application.LocalUserAppDataPath;
                    appDataTemp = appDataTemp.Substring(0, appDataTemp.IndexOf("ocal\\"));                       
                }
                string findTemp = appDataTemp.ToString();
                string tempDir = Path.Combine(findTemp, "Temp");
                if (Directory.Exists(findTemp))
                {
                    if (!Directory.Exists(tempDir))
                        Directory.CreateDirectory(tempDir);
                    if (!Directory.Exists(tempDir))
                        return SaveBytesDialog(fileBytes, ref outFilePath);

                    outFilePath = Path.Combine(tempDir, Path.GetFileName(outFilePath));
                    try
                    {
                        File.WriteAllBytes(outFilePath, fileBytes);
                    }
